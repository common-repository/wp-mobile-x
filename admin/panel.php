...
class MOBILE_X_PANEL{
    ...
    function init(){
        add_menu_page( __('Mobile Theme', 'wp-mobile-x'), __('Mobile Theme', 'wp-mobile-x'), 'edit_theme_options', 'wp-mobile-x', array( &$this, 'options'), 'dashicons-smartphone', '88');
        if (is_admin() && isset($_GET['page']) && ( $_GET['page'] == 'wp-mobile-x' ) ) {
            // Load CSS
            wp_enqueue_style( "mobx-panel", MobX_URL . "admin/css/panel.css", false, MobX_VERSION, "all");
            wp_enqueue_style( 'wp-color-picker' );

            // Load JS
            wp_enqueue_script("mobx-panel", MobX_URL . "admin/js/panel.min.js", array('jquery', 'jquery-ui-core', 'wp-color-picker'), MobX_VERSION, true);
            wp_enqueue_media();
        }
    }
    ...
    function options(){
        ...
        <div class="wrap wpcom-wrap">
            <div class="wpcom-panel-head">
                ...
                <h1><?php _e('Mobile Theme', 'wp-mobile-x');?><small>WP Mobile X</small></h1>
            </div>
    ...
    private function build_form(){
        ...
        <div class="wpcom-panel-form" id="j-panel-form">
            <ul class="wpcom-panel-nav">
                ...
                <?php } } ?>
                <?php if(get_locale()=='zh_CN'){ ?><li<?php echo ($i+1)==$active?' class="more-themes active"':' class="more-themes"'?>><i class="fa fa-magic"></i> <?php _e('More Themes', 'wp-mobile-x');?></li><?php } ?>
            </ul>
            <div class="wpcom-panel-content">
                <form action="" method="post" id="wpcom-panel-form">
                    ...
                    <div class="wpcom-panel-submit" style="display: none;">
                        ...
                        <input type="submit" name="submit" id="submit" class="button button-primary" value="<?php _e('Save Changes', 'wp-mobile-x');?>">
                    </div>
                </form>
                <?php foreach ($this->settings as $i => $item) { if($item===false){
                    $this->active_form($i);
                } } ?>
                <?php if(get_locale()=='zh_CN'){ ?><div class="wpcom-panel-item more-theme-wrap<?php echo $i+1==$active?' active':''?>"></div><?php } ?>
            </div>
        </div>
        <?php
    }
    ...
    private function option_item($option, $i, $repeat='-1'){
        ...
        switch ($type) {
            ...
            case 'page':
                $html = '<option value="">'.__('-- Please select --', 'wp-mobile-x').'</option>';
                $pages = MobX_Utils::get_all_pages();
            ...
            case 'repeat':
                ...
                echo '<div class="repeat-btn-wrap"><button type="button" class="button j-repeat-add" id="wpcom_'.$name.'"><i class="dashicons dashicons-plus"></i> '.__('Add more', 'wp-mobile-x').'</button></div></div>';
                break;
            case 'version':
                echo '<div class="form-group clearfix"><label class="form-label">'.$title.'</label><div class="form-input" style="padding-top:5px;">'.MX_THEME_VERSION.' <a class="check-version" id="j-check-version" href="javascript:;">'.__('Check update', 'wp-mobile-x').'</a>'.$notice.'</div></div>';
                break;
            default:
                break;
        }
